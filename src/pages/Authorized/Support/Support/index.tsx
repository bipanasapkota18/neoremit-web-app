  ...
  return (
    <HStack gap={6} width={"100%"} alignItems={"start"}>
      <Card minHeight={"88vh"} minWidth={"632px"}>
        <CardBody display={"flex"} flexDirection={"column"} gap={6} padding={6}>
          {flag ? (
            <SupportResponse
              setFlag={setFlag}
              ...
              <TabList gap={3}>
                {tabsList.map((tab, index) => (
                  <Tab
                    ...
                    borderRadius={"32px"}
                    _selected={{ color: "white", bg: colorScheme.primary_300 }}
                  >
                    {tab.title}
                  </Tab>
                ))}
      ...
      <Stack gap={6}>
        <Card>
          <CardBody display={"flex"} flexDir={"column"} padding={6} gap={6}>
            ...
            <Stack alignItems={"flex-end"} fontWeight={500} cursor={"pointer"}>
              <Text color={colorScheme.primary_500}>View More</Text>
            </Stack>
          </CardBody>
        </Card>
        <Card>
